...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkNotificationPermission = () => {
        ...
        if (!('Notification' in window)) {
            ...
            notificationPermissionAlert.style.display = 'block'; // Ensure it's visible
        ...
        if (Notification.permission === 'denied') {
            ...
            notificationPermissionAlert.style.display = 'block'; // Show alert
            reminderEnabledCheckbox.disabled = true;
        } else if (Notification.permission === 'granted') {
            notificationPermissionAlert.style.display = 'none'; // Hide alert
            reminderEnabledCheckbox.disabled = false;
        } else { // 'default' state
            // For 'default', we hide the alert but keep the checkbox enabled so user can request permission
            notificationPermissionAlert.style.display = 'none'; // Hide alert
            reminderEnabledCheckbox.disabled = false;
        }
    };
    ...
    const addTask = () => {
        ...
        const dueDate = null; // For now, due date is not set on creation
    ...
    const editTask = (listItem, taskSpan) => {
        ...
        // Create reminder UI elements
        const editReminderContainer = document.createElement('div');
        editReminderContainer.classList.add('edit-reminder-container');

        const editReminderEnabled = document.createElement('input');
        editReminderEnabled.type = 'checkbox';
        editReminderEnabled.id = 'edit-reminder-enabled';
        const editReminderLabel = document.createElement('label');
        editReminderLabel.htmlFor = 'edit-reminder-enabled';
        editReminderLabel.textContent = 'リマインダーを有効にする';
        editReminderContainer.appendChild(editReminderEnabled);
        editReminderContainer.appendChild(editReminderLabel);

        const editReminderOptions = document.createElement('div');
        editReminderOptions.classList.add('reminder-options', 'hidden');
        editReminderContainer.appendChild(editReminderOptions);

        const editReminderTypeSelect = document.createElement('select');
        editReminderTypeSelect.classList.add('edit-reminder-type-select');
        const reminderTypes = [{value: 'relative', text: '相対'}, {value: 'absolute', text: '絶対'}];
        reminderTypes.forEach(type => {
            const option = document.createElement('option');
            option.value = type.value;
            option.textContent = type.text;
            editReminderTypeSelect.appendChild(option);
        });
        editReminderOptions.appendChild(editReminderTypeSelect);

        const editReminderValueRelative = document.createElement('div');
        editReminderValueRelative.classList.add('reminder-value-relative');
        const editReminderAmount = document.createElement('input');
        editReminderAmount.type = 'number';
        editReminderAmount.id = 'edit-reminder-amount';
        const editReminderUnit = document.createElement('select');
        editReminderUnit.id = 'edit-reminder-unit';
        const units = [{value: 'minutes', text: '分前'}, {value: 'hours', text: '時間前'}, {value: 'days', text: '日前'}];
        units.forEach(unit => {
            const option = document.createElement('option');
            option.value = unit.value;
            option.textContent = unit.text;
            editReminderUnit.appendChild(option);
        });
        editReminderValueRelative.appendChild(editReminderAmount);
        editReminderValueRelative.appendChild(editReminderUnit);
        editReminderOptions.appendChild(editReminderValueRelative);

        const editReminderValueAbsolute = document.createElement('div');
        editReminderValueAbsolute.classList.add('reminder-value-absolute', 'hidden');
        const editReminderDatetime = document.createElement('input');
        editReminderDatetime.type = 'datetime-local';
        editReminderDatetime.id = 'edit-reminder-datetime';
        editReminderValueAbsolute.appendChild(editReminderDatetime);
        editReminderOptions.appendChild(editReminderValueAbsolute);


        // Populate reminder UI based on existing data
        const originalReminderRule = listItem.dataset.reminderRule ? JSON.parse(listItem.dataset.reminderRule) : null;
        const originalReminderAt = listItem.dataset.reminderAt;

        // Initialize values for all reminder inputs (always do this for consistency)
        editReminderEnabled.checked = false;
        editReminderOptions.classList.add('hidden');
        editReminderTypeSelect.value = 'relative'; // Default to relative
        editReminderAmount.value = '';
        editReminderUnit.value = 'minutes';
        editReminderDatetime.value = '';
        editReminderValueRelative.classList.remove('hidden'); // Default to relative visible
        editReminderValueAbsolute.classList.add('hidden'); // Default to absolute hidden


        if (originalReminderRule) {
            editReminderEnabled.checked = true;
            editReminderOptions.classList.remove('hidden');
            editReminderTypeSelect.value = originalReminderRule.type;
            if (originalReminderRule.type === 'relative') {
                editReminderAmount.value = originalReminderRule.value.amount;
                editReminderUnit.value = originalReminderRule.value.unit;
                editReminderValueRelative.classList.remove('hidden');
                editReminderValueAbsolute.classList.add('hidden');
            } else if (originalReminderRule.type === 'absolute') {
                if (originalReminderRule.value) { // Use originalReminderRule.value (ISO string) to populate datetime-local
                    const date = new Date(originalReminderRule.value);
                    const year = date.getFullYear();
                    const month = String(date.getMonth() + 1).padStart(2, '0');
                    const day = String(date.getDate()).padStart(2, '0');
                    const hours = String(date.getHours()).padStart(2, '0');
                    const minutes = String(date.getMinutes()).padStart(2, '0');
                    editReminderDatetime.value = `${year}-${month}-${day}T${hours}:${minutes}`;
                }
                editReminderValueRelative.classList.add('hidden');
                editReminderValueAbsolute.classList.remove('hidden');
            }
        }

        // Event listeners for reminder UI
        editReminderEnabled.addEventListener('change', () => {
            editReminderOptions.classList.toggle('hidden', !editReminderEnabled.checked);
        });

        editReminderTypeSelect.addEventListener('change', () => {
            const isRelative = editReminderTypeSelect.value === 'relative';
            editReminderValueRelative.classList.toggle('hidden', !isRelative);
            editReminderValueAbsolute.classList.toggle('hidden', isRelative);
        });
        ...
        taskDetails.prepend(editReminderContainer, editPrioritySelect, editTagInput, editInput);
        listItem.querySelector('.task-actions').prepend(finishEditButton);

        editInput.focus();

        const finishEditing = () => {
            console.log('--- finishEditing started ---');
            const newText = editInput.value.trim();
            const newTags = editTagInput.value.split(',').map(tag => tag.trim()).filter(tag => tag !== '');
            const newPriority = editPrioritySelect.value;

            // Get reminder rule from dynamically created UI BEFORE removing elements
            let newReminderRule = null;
            let newReminderAt = null;
            console.log('editReminderEnabled.checked:', editReminderEnabled.checked);
            if (editReminderEnabled.checked) {
                const type = editReminderTypeSelect.value;
                console.log('Reminder type:', type);
                if (type === 'relative') {
                    const amount = parseInt(editReminderAmount.value, 10);
                    const unit = editReminderUnit.value;
                    console.log('Relative amount:', amount, 'unit:', unit);
                    if (!isNaN(amount) && unit) { // Validate input
                        newReminderRule = { type: 'relative', value: { amount, unit } };
                    }
                } else if (type === 'absolute') {
                    const datetime = editReminderDatetime.value;
                    console.log('Absolute datetime (from UI):', datetime);
                    if (datetime) { // Validate input
                        // Convert datetime-local string to ISO string for consistent storage
                        newReminderRule = { type: 'absolute', value: new Date(datetime).toISOString() };
                    }
                }
                // Pass listItem.dataset.dueDate or a default if it might be null
                newReminderAt = calculateReminderAt(listItem.dataset.dueDate || new Date().toISOString(), newReminderRule);
                console.log('Calculated newReminderAt:', newReminderAt);
            }
            listItem.dataset.reminderRule = newReminderRule ? JSON.stringify(newReminderRule) : '';
            listItem.dataset.reminderAt = newReminderAt || '';
            console.log('Saved reminderRule:', listItem.dataset.reminderRule);
            console.log('Saved reminderAt:', listItem.dataset.reminderAt);
            ...
            // Remove reminder UI elements
            taskDetails.removeChild(editReminderContainer);

            listItem.classList.remove('editing');
            listItem.setAttribute('draggable', 'true');
            updateApp();
            console.log('--- finishEditing finished ---');
        };
